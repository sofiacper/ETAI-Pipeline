...
def preprocess(df, target, sensitive_attr, drop_columns, test_size, random_state):
    data_config = {
        "target": target,
    ...
    X, y, extras = split_features_target(df, data_config, mnar_indicator_sources=[])
    
    
    X_train, X_test, y_train, y_test, extras_train, extras_test = split_train_test(
        X, y, extras, test_size=test_size, random_state=random_state
    )
